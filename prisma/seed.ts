...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL ?? "admin@convergens.local";
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD ?? "ChangeMe123!"; // change after first login
...
async function ensureDefaultAdmin() {
  ...
  const admin = await prisma.organizer.upsert({
    ...
    update: { role: Role.ADMIN }, // ensure existing user is elevated if needed
    create: {
      email: ADMIN_EMAIL,
      passwordHash,
      role: Role.ADMIN,
  ...
  console.log(
    `✔ Default admin ensured: ${admin.email} (${admin.id}) [role=${admin.role}]`,
  );
}
