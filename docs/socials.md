...
La collection `socials` sert à enregistrer des liens vers les réseaux sociaux (YouTube, Facebook, Twitter, Instagram…) et à les associer à une **organisation** (collection `collectives`).
...
- **type** *(obligatoire)* : plateforme du réseau social  
  Choix disponibles :
  - `youtube` (YouTube)
  - `facebook` (Facebook)
  - `twitter` (Twitter)
...
- **url** *(obligatoire, unique)* : lien vers la page/profil
...
- **collective** *(obligatoire, M2O → `collectives`)* : organisation à laquelle appartient ce lien
...
### “Je ne vois pas mon organisation dans le champ collective”
- C’est normal si vous n’êtes pas listé·e dans `collectives.organizers` pour cette organisation.
